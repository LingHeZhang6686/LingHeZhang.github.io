...
if (exists("pca_result")) {
  ...
  tryCatch({
    ...
    # 保存PCA结果
    pdf("pca_plots.pdf", width = 12, height = 8)
    print(p1)
    print(p2)
...
tryCatch({
  ...
  ggsave("tsne_plot.pdf", p_tsne, width = 10, height = 8)
  cat("✓ t-SNE可视化已保存为 tsne_plot.pdf\n")
...
tryCatch({
  ...
  ggsave("umap_plot.pdf", p_umap, width = 10, height = 8)
  cat("✓ UMAP可视化已保存为 umap_plot.pdf\n")
  
}, error = function(e) {
  cat("UMAP错误:", e$message, "\n")
})

# 方法4：LASSO特征选择（处理高维数据）
cat("\n=== 开始LASSO特征选择 ===\n")
# 准备数据
...
tryCatch({
  cv_lasso <- cv.glmnet(x, y, 
                        ...
                        parallel = FALSE,  # 先关闭并行
                        type.measure = "class")
  
  # 可视化交叉验证结果
  pdf("lasso_cv.pdf", width = 8, height = 6)
...
# 后续分析继续使用important_data...

# 由于代码较长，这里只展示修复后的关键部分
# 剩余的分析方法（层次聚类、随机森林等）可以继续使用important_data进行分析
...
# 创建综合分析报告
create_analysis_report <- function(data, labels, pca_result) {
  results <- list()
  
  # 1. 数据基本信息
  results$data_info <- list(
    n_samples = nrow(data),
    n_features = ncol(data),
    n_classes = length(unique(labels$Class)),
    class_distribution = table(labels$Class)
  )
  
  # 2. PCA结果
  if (exists("pca_result")) {
    pca_var <- summary(pca_result)$importance[3, ]
    results$pca_summary <- list(
      n_components = length(pca_result$sdev),
      variance_explained = pca_var,
      components_for_80_percent = which(pca_var >= 0.8)[1],
      components_for_90_percent = which(pca_var >= 0.9)[1]
    )
  }
  
  # 3. 特征选择结果
  if (exists("important_features")) {
    results$feature_selection <- list(
      n_selected_features = length(important_features),
      selected_features = colnames(data)[important_features]
    )
  }
...
tryCatch({
  ...
  # 可视化特征重要性
  pdf("rf_importance.pdf", width = 12, height = 8)
  varImpPlot(rf_model, 
             main = "Random Forest - Feature Importance",
             cex = 0.8)
  dev.off()
  cat("✓ 随机森林特征重要性已保存为 rf_importance.pdf\n")
  ...
  ggsave("rf_top20_features.pdf", p_rf_top20, width = 10, height = 8)
  cat("✓ 前20个重要特征可视化已保存\n")
...
tryCatch({
  if (exists("important_data") && ncol(important_data) > 1) {
    ...
    # 方法1：使用pheatmap
    pdf("top_genes_heatmap_final.pdf", width = 14, height = 10)
    ...
    dev.off()
    cat("✓ 热图已保存为 top_genes_heatmap_final.pdf\n")
    ...
    # 绘制分组热图
    pdf("mean_expression_by_cancer.pdf", width = 12, height = 6)
    pheatmap(mean_expression_scaled,
             ...
             border_color = "gray")
    dev.off()
    cat("✓ 分组平均表达热图已保存为 mean_expression_by_cancer.pdf\n")
  ...
  cat("尝试最基本的heatmap函数...\n")
  tryCatch({
    pdf("basic_heatmap.pdf", width = 10, height = 8)
    ...
    dev.off()
    cat("✓ 基础热图已保存为 basic_heatmap.pdf\n")
  }, error = function(e2) {
    cat("最终热图失败:", e2$message, "\n")
  })
})

# 方法8：箱线图可视化重要基因表达（修复版）
cat("\n=== 开始重要基因表达分析 ===\n")
tryCatch({
  if (exists("imp_df") && nrow(imp_df) >= 5) {
    ...
    ggsave("top_genes_boxplot.pdf", p_boxplot, width = 14, height = 10)
    cat("✓ 基因表达箱线图已保存为 top_genes_boxplot.pdf\n")
  } else {
...
tryCatch({
  ...
  # 添加图例
  legend_plot <- ggplot(plot_df, aes(x = Dim1, y = Dim2, color = Class)) +
    geom_point(size = 2) +
    theme_void() +
    theme(legend.position = "right",
  ...
  # 使用patchwork组合图形
  combined_plot <- (plot_list$PCA | plot_list$tSNE | plot_list$UMAP | legend_plot) +
    plot_layout(widths = c(1, 1, 1, 0.5)) +
    plot_annotation(title = "Comparison of Dimensionality Reduction Methods",
                    theme = theme(plot.title = element_text(hjust = 0.5, 
                                                            face = "bold", 
                                                            size = 14)))
  
  ggsave("dim_reduction_comparison.pdf", combined_plot, 
         width = 16, height = 6)
  cat("✓ 降维方法比较图已保存为 dim_reduction_comparison.pdf\n")
...
tryCatch({
  ...
  # 绘制混淆矩阵
  pdf("confusion_matrix.pdf", width = 10, height = 8)
  ...
  print(p_cm)
  dev.off()
  cat("✓ 混淆矩阵已保存为 confusion_matrix.pdf\n")
...
tryCatch({
  if (ncol(important_data) > 1) {
    ...
    # 可视化相关性矩阵
    pdf("feature_correlation.pdf", width = 10, height = 8)
    corrplot(cor_matrix, 
             method = "color",
             type = "upper",
             tl.col = "black",
    ...
    dev.off()
    cat("✓ 特征相关性图已保存为 feature_correlation.pdf\n")
...
tryCatch({
  ...
  ggsave("pca_loadings_plot.pdf", p_loadings, width = 10, height = 8)
  cat("✓ PCA loadings图已保存为 pca_loadings_plot.pdf\n")
...
cat("✓ 最终汇总已保存为 final_analysis_summary.csv\n")

cat("🎉 基因表达数据分析完成！\n")
cat("📁 已生成的分析文件:\n")
cat("\n1. 📊 可视化文件 (PDF格式):\n")
cat("   ├── pca_plots.pdf           - PCA可视化\n")
cat("   ├── tsne_plot.pdf           - t-SNE可视化\n")
cat("   ├── umap_plot.pdf           - UMAP可视化\n")
cat("   ├── lasso_cv.pdf            - LASSO交叉验证\n")
cat("   ├── dendrogram.pdf          - 层次聚类树\n")
cat("   ├── rf_importance.pdf       - 随机森林特征重要性\n")
cat("   ├── rf_top20_features.pdf   - 前20个重要特征\n")
cat("   ├── top_genes_heatmap_final.pdf   - 重要基因热图\n")
cat("   ├── mean_expression_by_cancer.pdf   - 分组平均表达热图\n")
cat("   ├── top_genes_boxplot.pdf   - 基因表达箱线图\n")
cat("   ├── dim_reduction_comparison.pdf - 降维方法比较\n")
cat("   ├── confusion_matrix.pdf    - 混淆矩阵\n")
cat("   ├── feature_correlation.pdf - 特征相关性图\n")
cat("   └── pca_loadings_plot.pdf   - PCA loadings图\n")
cat("\n2. 📁 数据文件 (CSV格式):\n")
...
cat("   └── analysis_report.html   - HTML报告\n")
cat("\n4. 💾 工作空间:\n")
